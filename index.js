...
const generateUserId = () => {
  ...
  const maxId =
    users.length > 0 ? Math.max(...users.map((user) => parseInt(user.id))) : 0;
...
const generateContactId = () => {
  ...
  const maxId =
    contacts.length > 0
      ? Math.max(...contacts.map((contact) => parseInt(contact.id)))
      : 0;
  return (maxId + 1).toString();
};
...
app.get("/", (req, res) => {
  res.send(`
    <div style="display: flex; justify-content: center; align-items: center; height: 100vh; background-color: #f0f0f0;">
      <a href="/signup.html" style="text-decoration: none; font-size: 24px;">CLICK HERE FOR REGISTRATION</a>
    </div>
  `);
});
...
app.get("/api/contacts/:id", authenticateToken, (req, res) => {
  ...
  const contact = contacts.find(
    (contact) => contact.id === id && contact.userId === req.user.id
  );
...
app.delete("/api/contacts/:id", authenticateToken, (req, res) => {
  ...
  if (contactIndex === -1) {
    console.log(
      `Contact with ID ${id} not found or does not belong to the user.`
    );
    return res.status(404).json({ message: "Contact not found" });
  }
